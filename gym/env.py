...
class DeepRLGridEnv(Env):
    """
    For tabular RL, state space takes into account actual current position as well as
    goal position

    Args:
        bounds: Gridworld bounds
        res: Gridworld resolutions
        obstacles: Obstacles in Gridworld. If no obstacles, pass in None
        use_shaped: Whether to use shaped reward function
        wrap_arena: Whether to allow robot to wrap movement around arena edges
        render_mode: Whether to render state transitions and environment
    """
    def __init__(self, bounds, res, obstacles=None, use_shaped=False, wrap_arena=False, render_mode='human', randomize_start=False, randomize_goal=False):
        super().__init__()
        # Environment
        self.bounds = bounds
    ...
    def __init_render__(self):
        """
        Initialize rendering of environment
        """
    ...
    def init_start_goal(self, init=False):
        """
    ...
    def wrap(self, pos):
        """
        Wrap robot movement around map edges
        """
        # Wrap x
        new_pos = pos.copy()
    ...
    def step(self, action_index):
        """
        Take an action step in environment, collecting observation and reward.
        """
...
class QLGridEnv(Env):
    ...
    Args:
        ...
        obstacles: Obstacles in Gridworld. If no obstacles, pass in None
        render_mode: Whether to render state transitions and environment
        randomize_start: Whether to randomize starting position on reset
        randomize_goal: Whether to randomize goal position on reset
    """
    def __init__(self, bounds, res, obstacles=None, render_mode='human', randomize_start=False, randomize_goal=False):
        super().__init__()
        # Environment
        self.bounds = bounds
        self.max_dist = abs(bounds[0][1] - bounds[0][0]) + abs(bounds[1][1] - bounds[1][0])
    ...
    def reset(self):
        """
        Reset environment for a new episode, optionally randomizing robot and goal at start
        """
        # Initialize starting and goal positions
